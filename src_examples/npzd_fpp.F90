...
! Retrieve current (local) state variable values.
         n = cache%read (self%id_n%index) ! nutrient
         p = cache%read (self%id_p%index) ! phytoplankton
         z = cache%read (self%id_z%index) ! zooplankton
         d = cache%read (self%id_d%index) ! detritus

! Retrieve current environmental conditions.
         par = cache%read (self%id_par%index)          ! local photosynthetically active radiation
         I_0 = cache%read_hz (self%id_I_0%horizontal_index)  ! surface photosynthetically active radiation
...
! Set temporal derivatives
         cache%write (self%id_n%sms%sum_index) = cache%write (self%id_n%sms%sum_index) + ((dn)*self%rdt__)
         cache%write (self%id_p%sms%sum_index) = cache%write (self%id_p%sms%sum_index) + ((primprod - g - self%rpn*p - rpd*p)*self%rdt__)
         cache%write (self%id_z%sms%sum_index) = cache%write (self%id_z%sms%sum_index) + ((g - self%rzn*z - self%rzd*z)*self%rdt__)
         cache%write (self%id_d%sms%sum_index) = cache%write (self%id_d%sms%sum_index) + ((rpd*p + self%rzd*z - self%rdn*d)*self%rdt__)
...
! the change in nutrients (assuming constant C:N ratio)
         if (self%id_dic%index/=-1) cache%write (self%id_dic%sms%sum_index) = cache%write (self%id_dic%sms%sum_index) + ((self%dic_per_n*dn)*self%rdt__)

! Export diagnostic variables
         cache%write (self%id_dPAR%write_index) = par
         cache%write (self%id_PPR%write_index) = primprod*secs_pr_day
         cache%write (self%id_NPR%write_index) = (primprod - self%rpn*p)*secs_pr_day

! Leave spatial loops (if any)
      
   end subroutine do

   subroutine do_ppdd(self, cache,pp,dd)
      class (type_gotm_npzd), intent(in) :: self
      type (type_interior_cache),intent(inout) :: cache;;real(rke) ,dimension(:,:),intent(inout) :: pp,dd
...
! By assigning with dd (i%state_index,j%state_index) = dd (i%state_index,j%state_index) + (val)*self%rdt__;pp (j%state_index,i%state_index) = pp (j%state_index,i%state_index) + (val)*self%rdt__ as opposed to dd (i%state_index,j%state_index) = dd (i%state_index,j%state_index) + (val)*self%rdt__,
! assignments to dd(i,j) are automatically assigned to pp(j,i) as well.
         dd (self%id_n%state_index,self%id_p%state_index) = dd (self%id_n%state_index,self%id_p%state_index) + (primprod)*self%rdt__;pp (self%id_p%state_index,self%id_n%state_index) = pp (self%id_p%state_index,self%id_n%state_index) + (primprod)*self%rdt__                           ! snp
         dd (self%id_p%state_index,self%id_z%state_index) = dd (self%id_p%state_index,self%id_z%state_index) + (fpz(self%gmax,self%iv,p,z+self%z0))*self%rdt__;pp (self%id_z%state_index,self%id_p%state_index) = pp (self%id_z%state_index,self%id_p%state_index) + (fpz(self%gmax,self%iv,p,z+self%z0))*self%rdt__ ! spz
         dd (self%id_p%state_index,self%id_n%state_index) = dd (self%id_p%state_index,self%id_n%state_index) + (self%rpn*p)*self%rdt__;pp (self%id_n%state_index,self%id_p%state_index) = pp (self%id_n%state_index,self%id_p%state_index) + (self%rpn*p)*self%rdt__                         ! spn
         dd (self%id_z%state_index,self%id_n%state_index) = dd (self%id_z%state_index,self%id_n%state_index) + (self%rzn*z)*self%rdt__;pp (self%id_n%state_index,self%id_z%state_index) = pp (self%id_n%state_index,self%id_z%state_index) + (self%rzn*z)*self%rdt__                         ! szn
         dd (self%id_d%state_index,self%id_n%state_index) = dd (self%id_d%state_index,self%id_n%state_index) + (self%rdn*d)*self%rdt__;pp (self%id_n%state_index,self%id_d%state_index) = pp (self%id_n%state_index,self%id_d%state_index) + (self%rdn*d)*self%rdt__                         ! sdn
         dd (self%id_p%state_index,self%id_d%state_index) = dd (self%id_p%state_index,self%id_d%state_index) + (rpd*p)*self%rdt__;pp (self%id_d%state_index,self%id_p%state_index) = pp (self%id_d%state_index,self%id_p%state_index) + (rpd*p)*self%rdt__                              ! spd
         dd (self%id_z%state_index,self%id_d%state_index) = dd (self%id_z%state_index,self%id_d%state_index) + (self%rzd*z)*self%rdt__;pp (self%id_d%state_index,self%id_z%state_index) = pp (self%id_d%state_index,self%id_z%state_index) + (self%rzd*z)*self%rdt__                         ! szd

! If an externally maintained DIC pool is present, change the DIC pool according to the
! the change in nutrients (assuming constant C:N ratio)
         dn = - primprod + self%rpn*p + self%rzn*z + self%rdn*d
         if (self%id_dic%index/=-1) pp (self%id_dic%state_index,self%id_dic%state_index) = pp (self%id_dic%state_index,self%id_dic%state_index) + (self%dic_per_n*dn)*self%rdt__
